...
function buildExcludedPathspecs(): string {
    return EXCLUDED_DIFF_PATHS.map((path) => `':(exclude)${path}'`).join(' ');
}

function buildExcludedSectionCommand(sourceVar: string, label: string, linesVar: string): string {
...
export function buildAliasAc(): string {
...
export function buildAliasUndo(): string {
